...
public class GuidesMonitor {
	...
	public synchronized void request() throws InterruptedException {
		if (availableGuides > 0) {
			availableGuides--;
			clientForm.updateStatus("Führer angefordert. Verfügbar: " + availableGuides);
	...
	public synchronized void release() {
		if (availableGuides < MAX_GUIDES) {
			availableGuides++;
			clientForm.updateStatus("Führer zurückgegeben. Verfügbar: " + availableGuides);
			notifyAll();
			clientForm.updateAvailableGuides(availableGuides);
		}else
			clientForm.updateStatus("Max Anzahl an Führer erreicht!.");
